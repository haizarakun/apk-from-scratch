...
| `App.exit()` | アプリを閉じる |
| `App.notify("題名","文字")` | **通知**を出す（ステータスバーに表示。`notify` を許可に指定） |
| `alert("文字")` | そのまま使える（メッセージとして表示） |
| `localStorage` | データ保存（アプリを閉じても残る） |
| `fetch("https://…")` | 通信（インターネット許可は自動） |
| `navigator.geolocation.getCurrentPosition(f)` | **現在地**（`location` を許可に指定） |
| `navigator.mediaDevices.getUserMedia({video:true})` | **カメラ／マイク**の映像・音声（`camera` / `mic`） |
| `<input type="file" accept="image/*" capture>` | **写真を撮る／選ぶ**（システムのカメラ・ギャラリーが開く） |
| `navigator.vibrate(200)` | **バイブ**（`vibrate` を許可に指定） |

端末の機能を使うときは、フォームの **permissions** 欄に使うものをカンマ区切りで書きます
（例: `camera,location,notify`）。アプリ起動時に Android の許可ダイアログが自動で出ます。
※ **プッシュ通知**（サーバーから送る通知）は Google のサービス登録が必要なため未対応です。
アプリ内から出す通知（`App.notify`）は使えます。
...
これで、JSON の設計図なら「文字・ボタン・画像・一覧・通信」のアプリ、HTML なら
「フォーム・複数画面・保存・通信・カメラ・現在地・写真・通知・バイブ・共有」まで、
コードを書かずに（HTML はコピペで）ブラウザだけで作って更新までできます。
Bluetooth・NFC・バックグラウンド常駐・プッシュ通知（サーバー配信）は未対応です。
